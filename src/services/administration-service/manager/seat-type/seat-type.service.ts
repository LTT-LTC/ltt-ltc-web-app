...
const getSeatTypeListAsync = async (params: GetSeatTypeListInputDto) => {
  const { data } = await http.get<ApiResult<PagedResultDto<SeatTypeOutputDto>>>(`${getRoleScopedRootPath()}${path}`, {
    params,
  });
  return data.data;
};
...
const getSeatTypeByIdAsync = async (id: string) => {
  const { data } = await http.get<ApiResult<SeatTypeOutputDto>>(`${getRoleScopedRootPath()}${path}/${id}`);
  return data.data;
};
