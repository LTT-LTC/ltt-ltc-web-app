...
const createSeatTypeAsync = async (body: CreateSeatTypeInputDto) => {
    const { data } = await http.post<ApiResult<SeatTypeOutputDto>>(`${rootPath}${path}`, body);
    return data.data;
};

const updateSeatTypeAsync = async (id: string, body: UpdateSeatTypeInputDto) => {
    const { data } = await http.put<ApiResult<SeatTypeOutputDto>>(`${rootPath}${path}/${id}`, body);
    return data.data;
};

const deleteSeatTypeAsync = async (id: string) => {
